...
class ZhihuAPI:
    ...
    def load_json(self, url, url_type):
        """ 给定url及要获取的信息类型，返回得到的json对象 """
    ...
    def get_user_profile(self, user_id):
        """ 获取用户的profile，即个人信息 """
        pro_url = 'https://www.zhihu.com/api/v4/members/'+user_id + \
    ...
    def get_user_topics(self, user_id, offset, limit, gqueue):
        """ 获取用户关注的topic(话题) """
        topics_url = 'https://www.zhihu.com/api/v4/members/'+user_id + \
            '/following-topic-contributions?include=data[*].topic.introduction&offset='+str(
                offset)+'&limit='+str(limit)
        j = self.load_json(topics_url, 'topic')
        gqueue.put((offset, j))  # 将获取到的json对象和offset值放在一起，组成元组，放入队列gqueue中

    def get_user_questions(self, user_id, offset, limit, gqueue):
        """ 获取用户关注的topic(话题) """
        questions_url = 'https://www.zhihu.com/api/v4/members/'+user_id + \
            '/following-questions?include=data[*].created,answer_count,follower_count,author&offset='+str(
                offset)+'&limit='+str(limit)
        j = self.load_json(questions_url, 'question')
        gqueue.put((offset, j))

    def get_user_followers(self, user_id, offset, limit, gqueue):
        """ 获取用户关注的follwers (the ones that follow this user) """
        followers_url = 'https://www.zhihu.com/api/v4/members/'+user_id + \
            '/followers?include=data[*].answer_count,articles_count,gender,follower_count,is_followed,is_following,badge[?(type=best_answerer)].topics&offset='+str(
                offset)+'&limit='+str(limit)
        j = self.load_json(followers_url, 'follower')
        gqueue.put((offset, j))

    def get_user_followees(self, user_id, offset, limit, gqueue):
        """ 获取用户关注的follwees (the ones that this user follows) """
        followees_url = 'https://www.zhihu.com/api/v4/members/'+user_id + \
            '/followees?include=data[*].answer_count,articles_count,gender,follower_count,is_followed,is_following,badge[?(type=best_answerer)].topics&offset='+str(
                offset)+'&limit='+str(limit)
        j = self.load_json(followees_url, 'followee')
        gqueue.put((offset, j))

    def get_user_all(self, func, user_id, count):
        """ 获取用户某项信息的全部内容 """
    ...
    def get_user_all_followees(self, user_id, count, dqueue):
        all_followees = self.get_user_all(
            self.get_user_followees, user_id, count)
        dqueue.put(('followee', all_followees))

    def get_user_all_followers(self, user_id, count, dqueue):
        all_followers = self.get_user_all(
            self.get_user_followers, user_id, count)
    ...
    def get_user_all_questions(self, user_id, count, dqueue):
        all_questions = self.get_user_all(
            self.get_user_questions, user_id, count)
    ...
    def get_user(self, user_id):
        ...
        try:
            # 从profile中获取following_count，following_topic_count等
        ...
        queue = Queue.Queue()
        user_followees_thread = threading.Thread(
            target=self.get_user_all_followees, args=(user_id, following_count, queue))
        # user_followers_thread=threading.Thread(target=self.get_user_all_followers,args=(user_id,follower_count,queue))
        user_topics_thread = threading.Thread(
            target=self.get_user_all_topics, args=(user_id, topic_count, queue))
        user_questions_thread = threading.Thread(
            target=self.get_user_all_questions, args=(user_id, question_count, queue))
