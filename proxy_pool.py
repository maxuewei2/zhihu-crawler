...
class proxy_pool:
    ...
    def __init__(self, timeout=5):
        ...
        self.add_proxy_thread = threading.Thread(
            target=self.add_proxies)  # 爬取代理网站，获取代理的线程
    ...
    def str2proxy(self, s):
        """ 将字符串转换为代理字典 """
        return {'http': s, 'https': s}

    def proxy2str(self, proxy):
        """ 将代理转换成字符串，以在proxy_times字典中作为键使用 """
        return proxy['http']

    def add_times(self, proxy):
        """ 增加某代理的使用总次数记录 """
        try:
    ...
    def add_page_proxies(self, page_num):
        """ 解析代理网站的一个页面，获取页面中的代理 """
        session = requests.session()
        session.headers.update(self.headers)
        try:
            page_response = session.get(
                "http://www.ip181.com/daili/"+str(page_num)+".html", timeout=self.timeout)
        except Exception, e:
            logging.warning('can not get ip181 page '+str(page_num))
            return
        # 用正则表达式解析页面，获取代理列表
        pros = re.findall(
            r'<tr[^>]+?>\s+<td>([^<]+?)</td>\s+<td>(\d+)</td>\s+<td>[^<]+?</td>\s+<td>HTTP,HTTPS</td>.+?</tr>', page_response.text, re.S)
        # 以多线程测试解析到的代理
        threads = []
        for pro in pros:
            new_test_thread = threading.Thread(
                target=self.test_proxy, args=(pro[0], pro[1]))
